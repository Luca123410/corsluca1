...
// --- CONFIGURAZIONE ---
const TIMEOUT_MS = 15000; // 15s per gestire le ricerche multiple
...
const BASE_1337X = "https://1337xx.to"; // Mirror più affidabile
...
/* ===========================================================
   1. IL CORSARO NERO (Fondamentale per ITA)
   =========================================================== */
...
/* ===========================================================
   2. 1337x (La TUA logica, ma ottimizzata)
   =========================================================== */
async function search1337x(title, year) {
    // Puliamo il titolo dall'anno per costruire le varianti
    let cleanTitle = cleanString(title);
    if (year) cleanTitle = cleanTitle.replace(year, '').trim();

    const baseQuery = `${cleanTitle} ${year || ''}`.trim();

    // Usiamo le keyword più efficaci (non tutte 17, per evitare timeout)
    ...
    // Eseguiamo le ricerche in PARALLELO (più veloce) invece che sequenziale
    ...
    // Recuperiamo i magnet dei top 5
...
/* ===========================================================
   4. KNABEN & TORRENTGALAXY (Backup Globali)
   =========================================================== */
...
/* ===========================================================
   🔴 MAIN AGGREGATOR (Finalmente completo!)
   =========================================================== */
async function searchMagnet(title, year) {
    ...
    // Qui lanciamo TUTTO insieme.
    // - Corsaro usa il titolo pulito (senza anno) per trovare di più.
    // - Gli altri usano Titolo + Anno per precisione.
    const promises = [
        searchCorsaro(cleanTitle),      // <--- ORA C'È!
        search1337x(title, year),       // <--- Tua logica ottimizzata
    ...
    allResults.forEach(item => {
        ...
        if (hash) {
            // Se viene da Corsaro, ha sempre la precedenza
